...
class BandwidthScheduler:
    ...
    def update_shares(self) -> None:
        ...
        s3_upload_count = 0
        s3_download_count = 0
        s3_upload_node_counts: dict[int, int] = defaultdict(int)
        s3_download_node_counts: dict[int, int] = defaultdict(int)
        ...
        for transfer in self.transfers:
            for leg in transfer.active_legs:
                if leg.bottleneck == "RAM_LOCAL":
                    ...
                    s3_upload_count += 1
                    s3_upload_node_counts[leg.source_node_id] += 1
                elif leg.bottleneck == "S3_DOWNLOAD":
                    s3_download_count += 1
                    s3_download_node_counts[leg.dest_node_id] += 1

        for transfer in self.transfers:
            for leg in transfer.active_legs:
                if leg.bottleneck == "RAM_LOCAL":
                    spec = self.node_specs[leg.source_node_id]
                    # NVLink bandwidth is stored per-GPU, so aggregate node
                    # bandwidth scales with the number of GPUs. PCIe bandwidth is
