...
class Cache:
    ...
    def _make_room_ram(self, node_id: int, size: int) -> list[TransferLeg]:
        ...
        Raises:
            RuntimeError: If the item is larger than the total RAM capacity
                and cannot fit even after evicting all items.
        """
        ...
        # After exhausting all RAM items, verify the item actually fits.
        if self.ram_usage_bytes[node_id] + size > self.ram_capacity_bytes[node_id]:
            raise RuntimeError(
                f"Item size ({size} bytes) exceeds node {node_id} RAM capacity "
                f"({self.ram_capacity_bytes[node_id]} bytes) even after full eviction"
            )
